...
mod mac {
    ...
    const HELPERS: &[&str] = &[
        "procurement Helper (GPU)",
        "procurement Helper (Renderer)",
        "procurement Helper (Plugin)",
        "procurement Helper (Alerts)",
        "procurement Helper",
    ];
    ...
    fn bundle(app_path: &Path) {
        ...
        HELPERS.iter().for_each(|helper| {
            create_app(
                ...
                helper,
                &example_path.join("procurement_helper"),
                true,
            );
        });
    }
    ...
    fn create_info_plist(
        ...
        let info_plist = InfoPlist {
            ...
            cf_bundle_identifier: "com.comet.procurement".to_string(),
            cf_bundle_info_dictionary_version: "6.0".to_string(),
            cf_bundle_name: "Comet Procure".to_string(),
    ...
    pub fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        run_command(&["build", "--bin", "procurement"])?;
        run_command(&["build", "--bin", "procurement_helper"])?;
        bundle(&app_path);
        Ok(())
    }
}
